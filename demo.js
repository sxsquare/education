...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const tName = document.getElementById('inp_t_name');
  ...
  const contClass = document.getElementById('continue_class');
 ...
 //   const finSname = fullSName.split(' ')[0] || 'User';  
  ...
  //  const finTname = fullTName.split(' ')[0] || 'User';
    ...
    const title = contClass.value;
    const selects = `${com.value}-${beh.value}-${qua.value}-${pun.value}-${eng.value}`;
